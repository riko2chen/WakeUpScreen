...
> **关于权限。** 通知亮屏的基本功能只需要通知访问权限，不申请网络权限。
> 可选的自定义亮屏时长在 Android 9+ 使用无障碍权限；可选的蓝牙条件在 Android 12+ 需要「附近设备」授权。
> 重复提醒振动使用普通振动权限。新增功能默认关闭或沿用原有设置，不读取屏幕内容。

## 更多功能

以下功能均可自由开启或关闭：

- **提醒控制** — 单独选择提醒应用、自定义 5～1440 分钟间隔、独立亮屏时长和可选振动；系统省电机制可能延迟提醒
- **蓝牙条件** — 仅在检测到所选已配对设备处于连接状态时允许亮屏
