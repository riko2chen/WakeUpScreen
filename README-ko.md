...
| | 기능 | 설명 |
|---|---|---|
| :bell: | **즉시 켜짐** | 알림이 오는 순간 화면이 켜집니다. |
| :sun_with_face: | **주머니 모드** | 주머니나 가방 안에 있는 것을 감지해 화면을 켜지 않습니다. |
| :repeat: | **반복 알림** | 읽지 않은 알림이 남아 있는 동안 5~60분마다 다시 켜집니다. |
| :hourglass_flowing_sand: | **화면 켜짐 시간 지정** | 시스템 화면 꺼짐 시간 대신 5~30초만 켜 두고 끕니다. |
| :mag: | **앱 필터** | 어떤 앱이 화면을 켤 수 있는지 정확히 고릅니다. |
| :chart_with_upwards_trend: | **주의 통계** | 최근 30일 동안 화면은 켰지만 한 번도 확인되지 않은 앱을 기기 안에서 집계합니다. |
| :clipboard: | **알림 로그** | 모든 알림의 켜짐 여부와 막힌 규칙을 기록하고, 검사 과정 도표를 실시간으로 보여 줍니다. |
| :new_moon: | **다크 모드** | AMOLED 화면에 잘 어울리는 다크 인터페이스. |
| :closed_lock_with_key: | **인터넷 불필요** | 전부 기기 안에서 동작하며, 데이터를 모으지도 서버에 연결하지도 않습니다. |
| :zap: | **가볍게** | 차지하는 자원이 최소한이고 배터리 영향도 거의 없습니다. |
