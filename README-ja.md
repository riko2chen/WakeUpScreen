...
| 機能 | 説明 |
|---|---|
| **即時点灯** | 通知が届いた瞬間に画面が点灯します。 |
| **ポケットモード** | ポケットやバッグの中にあることを検知して、画面を消したままにします。 |
| **繰り返しリマインド** | 未読の通知が残っている間、5〜1440 分ごとに再点灯します。 |
| **点灯時間のカスタマイズ** | システムの画面消灯時間ではなく、5〜30 秒だけ点灯して消します。 |
| **アプリの絞り込み** | どのアプリに画面を点けさせるかを細かく選べます。 |
| **注目度の統計** | 過去 30 日間で画面を点けたのに一度も見られなかったアプリを端末内で集計します。 |
| **通知ログ** | すべての通知について点灯の有無とどのルールで止まったかを記録し、チェック処理の図もリアルタイムで表示します。 |
| **ダークモード** | AMOLED ディスプレイに馴染むダーク表示。 |
| **通信不要** | すべて端末内で完結し、データ収集もサーバー通信もありません。 |
| **軽量** | 占有リソースはごくわずかで、バッテリーへの影響もほとんどありません。 |
...
> **権限について。** 通知による点灯の基本機能には通知へのアクセスが必要です。インターネット権限は要求しません。
> 任意の点灯時間設定は Android 9 以降でユーザー補助を、Bluetooth 条件は Android 12 以降で「付近のデバイス」の権限を使います。
> リマインドの振動には通常の振動権限を使います。新機能は初期状態でオフ、または既存の設定を継承し、画面内容を読み取りません。

## その他の機能

いずれも自由にオン・オフできます。

- **リマインド設定** — 対象アプリ、5〜1440 分の間隔、専用の点灯時間、任意の振動を設定できます。省電力機能により遅れる場合があります
- **Bluetooth 条件** — 選択したペアリング済みデバイスの接続を検出している間だけ点灯を許可します
