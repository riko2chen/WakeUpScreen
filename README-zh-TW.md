...
| | 功能 | 說明 |
|---|---|---|
| :bell: | **即時亮螢幕** | 收到通知的瞬間螢幕自動亮起。 |
| :sun_with_face: | **口袋模式** | 偵測到手機在口袋或包包裡時，保持螢幕熄滅。 |
| :repeat: | **重複提醒** | 通知還沒處理完，每隔 5～60 分鐘再亮一次。 |
| :hourglass_flowing_sand: | **自訂亮螢幕時間** | 螢幕亮 5 到 30 秒後熄滅，不走系統的螢幕逾時。 |
| :mag: | **應用程式篩選** | 精確選擇哪些應用程式可以亮螢幕。 |
| :chart_with_upwards_trend: | **注意力統計** | 在本機統計最近 30 天裡點亮了螢幕卻從沒被看一眼的應用程式。 |
| :clipboard: | **通知記錄** | 記錄每則通知亮沒亮螢幕、被哪條規則擋下，並配有即時的檢查流程圖。 |
| :new_moon: | **深色模式** | 搭配 AMOLED 螢幕的深色介面。 |
| :closed_lock_with_key: | **無需網路** | 完全在本機執行，不蒐集資料，不連線伺服器。 |
| :zap: | **輕量** | 資源占用極小，幾乎無感的耗電量。 |
