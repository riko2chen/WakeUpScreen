...
> **關於權限。** 通知亮螢幕的基本功能只需要通知存取權限，不申請網路權限。
> 可選的自訂亮螢幕時間在 Android 9+ 使用無障礙權限；可選的藍牙條件在 Android 12+ 需要「附近裝置」授權。
> 重複提醒震動使用一般震動權限。新增功能預設關閉或沿用原有設定，不讀取螢幕內容。

## 更多功能

以下功能皆可自由開啟或關閉：

- **提醒控制** — 單獨選擇提醒應用程式、自訂 5～1440 分鐘間隔、獨立亮螢幕時間和可選震動；系統省電機制可能延遲提醒
- **藍牙條件** — 僅在偵測到所選已配對裝置處於連線狀態時允許亮螢幕
